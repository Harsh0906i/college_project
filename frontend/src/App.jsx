...
//     try {
//       setTimeout(async () => {
//         const response = await fetch("https://college-project-8cxh.onrender.com/chat", {
//           method: "POST",
//           headers: {
...
function App() {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 text-sm md:text-base">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, index) => (
          ...
          >
            {msg.sender === "bot" && (
              <img src={image} alt="Bot Avatar" className="w-8 h-8 md:w-10 md:h-10 rounded-full" />
            )}
            <div
              className={`p-2 md:p-3 rounded-lg max-w-xs md:max-w-sm flex flex-col ${msg.sender === "user"
                ? "bg-gray-200 text-black"
                : "bg-blue-500 text-white"
                }`}
            >
            ...
            {msg.sender === "user" && (
              <FaUser className="w-6 h-6 md:w-7 md:h-7 text-gray-600 shadow-lg rounded-full" />
            )}
          </div>
        ))}
      ...
      {/* FAQ Section */}
      {showFAQ && (
        <div className="fixed bottom-0 left-0 right-0 bg-white p-4 border-t max-h-60 overflow-y-auto">
          <div className="flex justify-between items-center">
            <h3 className="font-semibold shadow-lg text-base md:text-lg mb-4">Frequently Asked Questions</h3>
            <button
              onClick={closeFAQ}
              className="bg-red-500 text-white px-3 py-1 rounded-full hover:bg-red-600"
            >
